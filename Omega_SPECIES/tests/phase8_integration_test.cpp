// Phase 8 "Integration" — enterprise-grade integration + soak suite.
// Drives the full engine (genesis -> body -> mind -> soul -> language -> mirror ->
// challenge) through a long soak loop and asserts the cross-module invariants
// that define a working, integrated system:
//   * every monitored signal stays finite (no NaN/inf blow-ups),
//   * the identity genotype core never changes (coherence == 1.0),
//   * the physics world's energy stays bounded (symplectic),
//   * the agent keeps inventing grounded words (language emerges),
//   * the Kolmogorov emitter and ZKP handshake stay valid under load.
// A periodic serial-introspection line is logged every few thousand ticks —
// a bounded, measurable proxy for the 24h soak.
...
int main() {
  ...
  for (int t = 0; t < TICKS; t++) {
    // Genesis: evolve topology occasionally (heavy; every 40 ticks).
    if (t % 40 == 0) pm.tick();
    ...
    if (t % 1000 == 0) {
      ...
      if (!hs.verify(hs.public_key(), id, p)) any_nan = true;  // treat invalid proof as failure
    }
    ...
    if (!std::isfinite(rE) || !std::isfinite(F) || !std::isfinite(coh) || !std::isfinite(e) || !std::isfinite(res)) any_nan = true;
    if (e < emin) emin = e;
    if (e > emax) emax = e;

    // Serial introspection log (proxy for the 24h soak telemetry).
    if (t % 8000 == 0) {
      printf("[soak t=%d] F=%.3f coh=%.1f words=%d e=[%.1f,%.1f] trust=%.2f\n",
             t, F, coh, words, emin, emax, tb.trust());
    }
  }
  ...
  check(!any_nan, "integration: no NaN/inf across the whole soak (signals stayed finite)");
  check(coherence_breaches == 0, "integration: identity genotype core never changed (0 coherence breaches)");
  check(coherence.coherence_score(live) == 1.0f, "integration: final coherence score is 1.0 (self intact)");
  check(words > 0, "integration: language emerged — agent minted grounded words during soak");
  check(have_word && first_word[0] == 'w' && std::strchr(first_word, '-') != nullptr,
        "integration: first word is a well-formed self-reference token (w-...)");
  check(emax - emin < 0.20f * std::fabs(emax > 0 ? emax : 1.0f), "integration: physics energy stayed bounded (symplectic, <20% swing)");
  check(kc.passes() && kc_entropy_min > 5.0f, "integration: Kolmogorov emitter output entropy exceeded seed throughout");
  check(spa.equilibrium(), "integration: self-play copy-vs-copy reached equilibrium");

  // Bootstrap legacy snapshot of the integrated run.
  boot.snapshot(uni.world_hash(), emax, spa.winner(), kc.passes() ? 1u : 0u, (uint32_t)TICKS, "OMEGA-P8");
  uint8_t blob[64]; int blen = 0; boot.serialize(blob, &blen);
  check(blen == 36, "integration: bootstrap legacy artifact serialized (36 bytes)");

  printf("\nPHASE8_INTEGRATION_TEST: %s (%d pass, %d fail)\n",
